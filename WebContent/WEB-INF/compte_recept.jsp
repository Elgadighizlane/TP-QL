...
<input type="hidden" name="deconnecterrecpt" value="deconnecterrecpt" />
	...
	<form method="post" action="compterecption">
<ul class="form-style-1">

<li><label>CIN </label><input id="cin" type="text" value="${sessionScope.session_receptionniste.cin}"  name="cin"  placeholder="CIN" disabled="true" /> </li>

    <li><label>Nom complete </label><input value="${sessionScope.session_receptionniste.nom}"  type="text" id="nom" name="nom" class="field-divided" placeholder="First" disabled="true"/> <input type="text" name="prenom" id="prenom" value="${sessionScope.session_receptionniste.prenom}" class="field-divided" placeholder="Last" disabled="true" /></li>
    <li>
        <label>Nom et ville de l'Hotel </label>
        <input type="text" name="nom_hotel" id="nom_hotel" value="${sessionScope.session_receptionniste.nom_hotel}" class="field-divided" disabled="true" />
   
        <input type="text" name="ville" id="ville" value="${sessionScope.session_receptionniste.ville_hotel}" class="field-divided" disabled="true" />
    </li>
     <li>
        <label>numero téléphone </label>
        <input type="text" name="tele" id="tele" value="${sessionScope.session_receptionniste.telephone}" class="field-long" disabled="true" />
    </li>
     <li>
        <label>Email </label>
        <input type="email" id="email" name="email" value="${sessionScope.session_receptionniste.email}" class="field-long" disabled="true"/>
    </li>
     <li>
        <label>addresse </label>
        <input type="text" name="addresse" id="addresse" value="${sessionScope.session_receptionniste.addresse}" class="field-long" disabled="true" />
    </li>
     <li>
        <label>mot de passe </label>
        <input type="password" name="pass" id="pass" value="${sessionScope.session_receptionniste.mot_passe}" class="field-long" disabled="true" />
    </li>
    <li>
        <label>mot de passe </label>
        <input type="password" name="pass2" id="pass2" value="${sessionScope.session_receptionniste.mot_passe}" class="field-long" disabled="true" />
    </li>
    
    <li>
        <input type="hidden" name="enregistrer" id="enregistrer" value="enregistrer" class="form-style-1"  />

<input type="hidden" onclick="location.reload();" id="cancel"  value="cancel" class="form-style-1" />
    </li>

</form>

<li>
<input type="button" onclick="enable_input()" id="modi" value="MODIFIER" class="form-style-1" />
...
<p>${erreur}</p>
    ...
    <script>  
        function enable_input() { 
            $("#tele").prop('disabled', false);
            $("#email").prop('disabled', false);
            $("#addresse").prop('disabled', false);
            $("#pass").prop('disabled', false);
            $("#pass2").prop('disabled', false);


            $("#cancel").prop('type', 'button'); 
             $("#enregistrer").prop('type', 'submit'); 
             $("#modi").prop('type', 'hidden'); 
        }
...
.form-style-1 input[type=submit],.form-style-1 button, .form-style-1 input[type=button]{
    background: #4B99AD;
    padding: 8px 15px 8px 15px;
    border: none;
    color: #fff;
}
.form-style-1 input[type=submit]:hover,.form-style-1 button, .form-style-1 input[type=button]:hover{
    background: #4691A4;
    box-shadow:none;
    -moz-box-shadow:none;
